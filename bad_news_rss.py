...
def build_item_html(
    *,
    ...
    if video_source:
        video_type = guess_media_type(video_source)
        iina_link = make_iina_link(video_source)
        poster_attr = f' poster="{escape(image)}"' if image else ""
        parts.append(
            f'<p><video controls preload="metadata"{poster_attr} '
            f'src="{escape(video_source)}" type="{escape(video_type)}"></video></p>'
        )
...
def build_rss(items: list[FeedItem], source_url: str, title: str) -> ET.ElementTree:
    ...
    for item in items:
        ...
        if item.video_source:
            media_type = guess_media_type(item.video_source)
            ET.SubElement(
                node,
                "enclosure",
                {"url": item.video_source, "type": media_type, "length": "0"},
            )
            media_content = ET.SubElement(
                node,
                "{http://search.yahoo.com/mrss/}content",
                {"url": item.video_source, "type": media_type, "medium": "video"},
            )
